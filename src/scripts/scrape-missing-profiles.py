#!/usr/bin/env python3
"""
Instagram HD Scraper for Missing/Failed Profiles
Scrapes only profiles without Cloudinary images
"""
...
class MissingProfilesScraper:
    def __init__(self, limit=100, proxy=None):
        self.limit = limit
        self.client = Client()
        ...
        session_id = '77630466294%3AmmwpwOFxujSnYL%3A1%3AAYgtubPI-r8Rxe5R5-8kzF0CnfPjeNVQ4oxOlpLz5A'
    ...
    def get_missing_profiles(self):
        """Get profiles without Cloudinary images"""
        try:
            ...
            query = f"""
                ...
                WHERE p.status = 'published'
                    ...
                    AND (p.image_url IS NULL
                         OR p.image_url = ''
                         OR p.image_url NOT LIKE '%cloudinary%')
                ORDER BY p.id
            ...
            print(f"Debug: Fetched {len(profiles)} profiles")
            if profiles:
                print(f"Debug: First profile: {profiles[0]}")
    ...
    def run(self):
        ...
        print("=" * 70)
        print("Instagram HD Scraper - Missing Profiles Only")
        print("=" * 70)
        print()

        # Get profiles
        profiles = self.get_missing_profiles()
        self.stats['total'] = len(profiles)

        print(f"📊 Found {self.stats['total']} profiles without Cloudinary images\n")
        print("=" * 70)
        print()
        ...
        for i, (profile_id, name, image_url, instagram_url) in enumerate(profiles, 1):
            ...
            if hd_pic_url == "CHALLENGE_REQUIRED":
                ...
                print()
                import random
                time.sleep(random.randint(45, 60))  # Longer delay after challenge
                continue
            ...
            if i < self.stats['total']:
                import random
                delay = random.randint(30, 45)  # Random delay 30-45 seconds
...
def main():
    # Get limit and proxy from command line
    limit = int(sys.argv[1]) if len(sys.argv) > 1 else 100
    proxy = sys.argv[2] if len(sys.argv) > 2 else None

    print(f"\n🎯 Processing up to {limit} profiles without Cloudinary images")
    if proxy:
        print(f"🌐 Using proxy: {proxy}")
    print()

    scraper = MissingProfilesScraper(limit=limit, proxy=proxy)
